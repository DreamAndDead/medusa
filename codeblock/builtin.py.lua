...
assert((all(list {_null()}) == true))
assert((all(list {_null(1, 2)}) == true))
assert((all(list {_null(0, 1, 2)}) == false))
assert((all(list {_null(0, 0)}) == false))
assert((any(list {_null()}) == false))
assert((any(list {_null(1, 2)}) == true))
assert((any(list {_null(0, 1, 2)}) == true))
assert((any(list {_null(0, 0)}) == false))
assert((bin(0) == "0b0"))
...
assert(bool(1))
assert(bool("abc"))
assert(bool(list {_null(1, 2)}))
assert(not bool(false))
assert(not bool(list {_null()}))
assert(not bool(""))
assert(not bool(0))
assert(callable(abs))
...
assert(not callable(list {_null()}))
local d = dict()
assert((len(d) == 0))
d[1] = "a"
d[2] = "b"
...
d, m = divmod((-10), (-3))
assert(((d == 3) and (m == (-1))))
local l = list {_null(0, 1, 2, 3, 4)}
for _, i, n in enumerate(l) do
    assert((i == n))
end
l = list {_null(0, 1, 2, 3, 4, 5)}
...
assert((len("") == 0))
assert((len("abcd") == 4))
assert((len(list {_null()}) == 0))
assert((len(list {_null(1, 2, 3)}) == 3))
assert((len(dict {}) == 0))
assert((len(dict {[0] = 0, ["a"] = "z"}) == 2))
assert((len(list()) == 0))
l = list(list {_null(1, 2, 3)})
assert((len(l) == 3))
assert((l[0] == 1))
assert((l[1] == 2))
assert((l[2] == 3))
l = list {_null(0, 1, 2, 3, 4)}
...
assert((sl[4] == 6))
assert((sl[5] == 9))
assert((sum(list {_null(1, 2, 3)}) == 6))
assert((sum(list {_null()}) == 0))
assert((sum(list {_null(1, 2, 3)}, 0) == 6))
assert((sum(list {_null(1, 2, 3)}, 4) == 10))
local t = tuple()
assert((len(t) == 0))
t = tuple(list {_null(1, 2, 3, 4)})
assert((t[0] == 1))
assert((t[1] == 2))
assert((t[2] == 3))
assert((t[3] == 4))
local l1 = list {_null(0, 1, 2, 3, 4, 5)}
local l2 = list {_null("a", "b", "c")}
local l3 = list {_null(3, ".", 1, 4, 1)}
