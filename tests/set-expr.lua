...
b = (function() local result = set {} for _, i in a do if bool((len(i) == 2)) then for _, j in i do if bool((j == 0)) then result.add(j) end end end end return result end)()
assert(bool((len(b) == 1)))
assert(bool((operator_in(0, b))))
c = (function() local result = set {} for _, i in a do if bool((len(i) == 2)) then for _, j in i do if bool((j == 0)) then result.add(bool((j == 1)) and j or 10) end end end end return result end)()
assert(bool((len(c) == 1)))
assert(bool((operator_in(10, c))))
return {
    a = a,
